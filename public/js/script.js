var playerM, myTimer, playerL, playerV, vimContr;
...
jQuery(document).ready(function ($) {
    ...
    $(document).on('click', '.popup_note .btn', function (e) {
        $('.popup_note').fadeOut();
    });
    ...
    $(document).on('click', '.play_control', function (e) {
        ...
        if ($(this).closest('.video_wrap').hasClass('video_playing')) {
            ...
            if($('.modal.open').hasClass('vimeo') && playerV !== undefined) playerV.pause();
            ...
            if($('.modal.open').hasClass('vimeo') && playerV !== undefined) playerV.play();
    ...
    $(document).on('click', '.sound_control', function () {
        ...
        if(!$(this).hasClass('muted')) {
            ...
            if($('.modal.open').hasClass('vimeo') && playerV !== undefined) playerV.setVolume(1);
            if($('.modal.open').hasClass('local')) playerL.muted = true;
        } else {
            if($('.modal.open').hasClass('youtube') && playerM !== undefined) playerM.mute();
            if($('.modal.open').hasClass('vimeo') && playerV !== undefined) playerV.setVolume(0);
    ...
    var updatePos = function (x, elem) {
        ...
        if($('.modal.open').hasClass('vimeo') && $('.video_wrap').hasClass("video_playing"))  {
            playerV.getDuration().then(function (duration) {
                playerV.setCurrentTime(percentage*duration);
            });
        }
    ...
    $(document).on('click', '.share_link', function (e) {
        e.preventDefault();
        var thL = $(this);
        var tempCopy = $("<input type='text' readonly='true'>");
...
function videoYT(id) {
    ...
    function onPlayerReady() {
        ...
        playerM.playVideo();
        if ($('#jsModal').hasClass('video3D') &&  window.innerWidth > 1199) {
            setTimeout(function () {
                $('.video_info').fadeOut();
            }, 1500)
        }
...
function videoVimeo(id) {
    ...
    vimContr = false;
    if ($('#jsModal').hasClass('video3D'))  vimContr = true;
    var options = {
        id: id,
        controls: vimContr,
        muted: true
    ...
    playerV = new Vimeo.Player('video_player', options);

    playerV.on('timeupdate', function (data) {
        var volume = $('.video-progress');

        volume.find('.draggable-point').css("left", data.percent * 100 + "%");
        volume.find('.video-progress-bar').css("width", data.percent * 100 + "%");
    });
    ...
    playerV.on('ended', function(data) {
        clearInterval(myTimer);
        setTimeout(function () {
            closeModal();
        }, 500)
    ...
    playerV.ready().then(function() {
        $('.modal.open .video_wrap').addClass('video_playing');
        playerV.setVolume(0);
        $('.sound_control').addClass('muted');
        playerV.play().then(function (id) {
            return playerV.play();
        });
        if ($('#jsModal').hasClass('video3D') &&  window.innerWidth > 1199) {
            setTimeout(function () {
...
function openModal(videoData, callback) {
    ...
    if(videoData.videoType3D === 'threeD') {
        ...
        if(window.innerWidth > 1199) { document.getElementsByClassName('video_info_desktop')[0].style.display = 'block';}
        else {document.getElementsByClassName('video_info_mobile')[0].style.display = 'block';}
    }
    ...
    if(videoData.videoType === 'vimeo') videoVimeo(videoData.videoId);
...
function closeModal(){
    ...
    if (modalWindow.classList.contains('vimeo')){
        playerV.pause();
        playerV.destroy();
        playerV = null;
    }
    ...
    document.getElementsByClassName('video_info_mobile')[0].style.display = 'none';

    modalWindow.classList ? modalWindow.classList.remove('open', 'modal_sm', 'local', 'youtube', 'vimeo', 'video3D', 'videoHTML', 'videoHTMLloop') : modalWindow.className = modalWindow.className.replace(new RegExp('(^|\\b)' + 'open'.split(' ').join('|') + '(\\b|$)', 'gi'), ' ');
