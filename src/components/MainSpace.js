...
import {TextureTypeEnum, gridOptions, sceneOptions} from '../configs/config'
...
export default class MainSpace {
    ...
    initMainSpace() {
        ...
        for( let step in this.steps ) {
            ...
            if( step !== 'intro' && step !== 'end' ) {
                ...
                for (let i = 0; i < Object.keys(this.assets.textures[ step ]).length; i++) {
                    ...
                    if(targetTextureData.type !== TextureTypeEnum.grid){
                        ...
                        // if(this.items[id].type === TextureTypeEnum.roadBlock) //if it's roadBlockItem
                            // this.roadBlocks.push({
                            //     videoBox: this.items[id].videoBox,
                            //     zPos: (this.items[id].position.z + nextStepLocation) * -1,
                            //     data: {},
                            // });
                        ...
                        targetTextureData.textures.sort((a, b)=> {
                    ...
                    zPosition = this.items[id].zPosition + 1000 + ( this.moreSpace ? this.moreSpace : 0 );
            ...
            nextStepLocation += bbox.min.z - posOffset - ( this.moreSpace ? this.moreSpace : 0 );
    ...
    updatePerspective() {
        ...
        TweenMax.to( this.camera.rotation, 4, {
            x: -this.mousePerspective.y * 0,
            y: -this.mousePerspective.x * 0,
            ease: 'Power4.easeOut',
        })
    ...
    handleVideos() {
        ...
        for( let i = 0; i < this.videoCount; i++ ) {
            ...
            if ( !this.frustum.intersectsObject( this.videoItems[ i ] ) && !this.videoItems[ i ].material.uniforms.texture.value.image.paused ) {
                if(!this.videoItems[ i ].overridePos)
    ...
    animate() {
        ...
        if( this.c.allowScrolling && this.c.scrolling && this.started ) {
            ...
            //
            let delta = ( this.c.scrollPos - this.mainSpace.position.z ) / 12
            if( this.grids.length && this.c.scrollPos >= this.grids[0].stops[0] && this.c.scrollPos <= this.grids[0].stops[1] ){                
            ...
            if( !this.c.isMobile && Math.abs( delta ) < 1000 ) this.handleVideos()
            this.changeColours()
    ...
    resize() {
        ...
        setTimeout(function () {
            ...
            th.camera.fov = 180 * ( 2 * Math.atan( th.c.size.h / 2 / th.camera.position.z ) ) / Math.PI
            th.camera.aspect = th.c.size.w / th.c.size.h
            th.camera.updateProjectionMatrix()
            th.renderer.setSize( th.c.size.w, th.c.size.h )
        }, 700)
    }
